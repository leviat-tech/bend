...
var draw = {
  ...
  bend: function bend(_ref12) {
    ...
    return {
      ...
      commands: [{
        type: 'arcto',
        params: [r, r, 0, 0, sf, position.x, position.y]
      }]
    };
  },
...
Bend.prototype.print = function print() {
  ...
  return this.commands().map(function (command) {
    var params = invertY ? invertParams(command).params : command.params;
    return [cmds[command.type]].concat(_toConsumableArray(params)).join(' ');
  }).join(' ');
};
